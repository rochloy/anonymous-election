...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabaseServer.rpc('release_digital_voting_reservation', {
    ...
    if (!result?.o_success) {
      return NextResponse.json({ success: false, message: result?.o_message || 'Release failed.' }, { status: 400 });
    }
