...
- **Paper Ballot Domain** (`paper_ballots`, `vote_audit_log`): Issued ballot tracking + audit
  - **Note:** `paper_ballots.status='VOTED'` is unreachable on the identity plane. Paper vote recording (`submit_paper_vote`) is member-blind and writes only to `ballots` (anonymous) and `anonymous_paper_blanks` (pool). The `PAPER_VOTED` dashboard badge is therefore never reached; `PAPER_ISSUED` remains the terminal state for checked-in paper voters. Tally is derived solely from `ballots` (anonymous). See `docs/plans/2026-09-17-v0.13.0-wave6-paper-severance.md` §N2.
...
36. `supabase/migration_wave8_voting_entitlement_provision.sql` — **v0.14.0 Wave 8 — FINAL writer for `check_in_paper_voter`.** Adds `private/public.ensure_voting_entitlement` + `private/public.provision_voting_entitlements` (SECURITY DEFINER, service_role only) and amends `check_in_paper_voter` to lazy-create the entitlement when absent (`channel_sent='NONE'`). Entitlement guard = zero non-voided VOTING tokens; outcomes 0→create / 1 unused→exists / 1 used→consumed / >1→integrity error. Batch audit routes through `insert_audit_log` (SEC-18 hash chain). Runs after item 35.
37. `supabase/migration_wave9_allow_member_add_during_voting.sql` — **Wave 9 — configurable late member addition during VOTING.** Adds `election_settings.allow_adding_member_during_voting` (default FALSE) + `private.assert_member_creation_allowed()`; `create_member` swaps its gate to the new assert (VOTING allowed only when the flag is on). `set_member_active` keeps `assert_electorate_editable()` (NOT relaxed); bulk CSV import keeps its own API-layer gate (NOT relaxed). The setting itself is only changeable during SETUP/NOMINATION/NOMINATION_CLOSED/VOTING (settings API layer). Runs after item 36.
