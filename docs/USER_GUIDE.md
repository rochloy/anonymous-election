...
- **Add Member**: Fill in Name (**required**); Email, Phone, and Member Code are optional. If you leave Member Code blank the system generates one automatically. A duplicate Member Code, Email, or Phone is rejected with a "member already exists" message. New members are added as active.
  - **Age eligibility (when the Voter Age Requirement is enabled):** two mutually exclusive optional inputs — **Date of Birth** (the system derives age eligibility; the DOB is never stored) or the **Age eligible** checkbox (assert eligibility without providing a DOB — the check-in-friendly path). Filling one disables the other; providing both is rejected. When the age requirement is disabled, neither input appears (everyone is eligible by default).
- **CSV Import**: Paste CSV with columns: `full_name` (or `name`) **required**; `email`, `phone`, `member_code`, `dob` (or `date_of_birth`) optional. Members without email can vote via paper ballots.
  - **DOB format:** use **ISO 8601 (`YYYY-MM-DD`)** — e.g. `1990-05-15`. ⚠ European-style `DD/MM/YYYY` is **silently misparsed** as US `MM/DD/YYYY` (`03/05/1990` becomes March 5, not May 3), which can flip age eligibility for members near the threshold. The DOB is used only when the Voter Age Requirement is enabled and is **never stored** — only the derived eligibility is kept. Missing/unparseable DOB → UNDETERMINED (configurable in Election Settings).
